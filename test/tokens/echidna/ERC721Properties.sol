    ...
    CryticERC721ExternalMintableProperties
{
    ...
    constructor() {
        ...
        token = IERC721Internal(vyperDeployer.deployContract("src/snekmate/tokens/mocks/", "erc721_mock", args));
        mockSafeReceiver = new MockReceiver(true);
        mockUnsafeReceiver = new MockReceiver(false);
    }

    function test_ERC721_external_mintIncreasesSupply(uint256 amount) public override {
        amount = clampBetween(amount, 0, 64);
        super.test_ERC721_external_mintIncreasesSupply(amount);
    }

    function test_ERC721_external_mintCreatesFreshToken(uint256 amount) public override {
        amount = clampBetween(amount, 0, 64);
