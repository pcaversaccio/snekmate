...
contract MessageHashUtilsTest is Test {
    ...
    function setUp() public {
        messageHashUtils = IMessageHashUtils(
            vyperDeployer.deployContract("src/snekmate/utils/mocks/", "message_hash_utils_mock")
        );
    ...
    function testToDataWithIntendedValidatorHashSelf() public view {
        ...
        bytes32 digest1 = messageHashUtils.to_data_with_intended_validator_hash_self(data);
        bytes32 digest2 = keccak256(abi.encodePacked(hex"19_00", messageHashUtilsAddr, data));
        assertEq(digest1, digest2);
    }
    ...
    function testToDataWithIntendedValidatorHash() public {
        ...
        bytes memory data = new bytes(42);
        bytes32 digest1 = messageHashUtils.to_data_with_intended_validator_hash(validator, data);
        bytes32 digest2 = keccak256(abi.encodePacked(hex"19_00", validator, data));
        assertEq(digest1, digest2);
    }
    ...
    function testToTypedDataHash() public view {
        ...
        bytes32 digest1 = messageHashUtils.to_typed_data_hash(domainSeparator, structHash);
        bytes32 digest2 = keccak256(abi.encodePacked(hex"19_01", domainSeparator, structHash));
        assertEq(digest1, digest2);
    }
    ...
    function testFuzzToDataWithIntendedValidatorHashSelf(bytes calldata data) public view {
        bytes32 digest1 = messageHashUtils.to_data_with_intended_validator_hash_self(data);
        bytes32 digest2 = keccak256(abi.encodePacked(hex"19_00", messageHashUtilsAddr, data));
        assertEq(digest1, digest2);
    }

    function testFuzzToDataWithIntendedValidatorHash(address validator, bytes calldata data) public view {
        bytes32 digest1 = messageHashUtils.to_data_with_intended_validator_hash(validator, data);
        bytes32 digest2 = keccak256(abi.encodePacked(hex"19_00", validator, data));
        assertEq(digest1, digest2);
    }

    function testFuzzToTypedDataHash(string calldata domainSeparatorPlain, string calldata structPlain) public view {
